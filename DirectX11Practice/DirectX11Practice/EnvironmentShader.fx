...
Texture2D objShadowMap1: register(t4);
Texture2D objShadowMap2: register(t5);
TextureCube EnvironmentCubeMap: register(t6);

SamplerState objSamplerState : register(s0); //WRAP
SamplerState objSamplerClamp : register(s1); //CLAMP
SamplerState EnvironmentSamplerState : register(s2); //WRAP for environment mapping
...
PS_INPUT EnvironmentVS(float4 inPos : POSITION, float2 inTexCoord : TEXCOORD, float3 normal : NORMAL, float3 tangent : TANGENT)
{
	PS_INPUT output;
...
float4 EnvironmentPS(PS_INPUT input) : SV_TARGET
{
	...
	float4 textDiffuse = float4(0.75f, 0.75f, 0.75f, 1.0f);
	if (HasTexture)
	...
	float3 toEye = CamPos - input.WorldPos;
	toEye /= length(toEye);
	float3 incident = -toEye;

	float3 reflectionVector = reflect(incident, input.Normal);
	float4 reflectionColor = EnvironmentCubeMap.Sample(EnvironmentSamplerState, reflectionVector);
	...
	finalColor = saturate(flashlightColor + finalAmbient + reflectionColor * float3(0.8f,0.8f, 0.8f));
...
float3 SpotLight(PS_INPUT input, s_Light light) :SV_Target
{
	...
	if (lightRatio > 0)
	{
		finalColor = temp * light.Diffuse  * light.Ambient / (light.Attenuation[0] + (light.Attenuation[1] * d) + (light.Attenuation[2] * (d * d)));
		lightRatio =  spotFactor * lightRatio / (light.Attenuation[0] + (light.Attenuation[1] * d) + (light.Attenuation[2] * (d * d)));

		reflection = normalize(1.0f * lightRatio * input.Normal - light.Dir);
		specular = temp * pow(saturate(dot(reflection, normalize(CamPos.xyz - input.WorldPos.xyz))), 260.0f);
		finalColor = saturate(finalColor * lightRatio + specular);
	}
